...
def get_blocks(frame_info_list):
    frame, f_label, f_idx = frame_info_list
    frame_block_list = []
    f_idx = 0
    ...
    for in_block in input_blocks:
        ...
        for i in range(16):
            ...
            op = cv2.GaussianBlur(in_block, k_size, sigmaX_list[i % 8])
            op_list.append(op)
        block_list.append([np.stack(op_list), f_label, f_idx])
    frame_block_list.append(block_list)
    f_idx += 1
...
def get_frame(video_info_list):
    video, v_label, v_idx = video_info_list
    frame_list = []
    ret_val = 1
    count = 0
    frame_num = 0
    ...
    while ret_val:
        ...
        count += 1
        if count % 5 == 0:
            f_idx = str(v_idx) + '_' + str(frame_num)
            frame_list.append([image, v_label, f_idx])
            frame_num += 1

    return frame_list, frame_num - 1
